...
def get_filter_coeffs(window_size):
    n_coeffs = (window_size // 2) + 1
    start = np.zeros(n_coeffs)
    end = np.zeros(n_coeffs)

    start[0:64] = np.hamming(64)

    end[30:90] = np.hamming(60)


    lines = []
    for i in range(n_coeffs):
        lines.append(np.linspace(start[i], end[i], num=64)[None, :])
