...
class DilatedChannelGenerator(nn.Module):
    def __init__(self, input_size, target_size, in_channels, channels, fb, sl,
        ...
        self.embedding_layer = \
            weight_norm(
                nn.Conv1d(in_channels, channels, 1, 1, bias=False))
    ...
    def forward(self, x):
        ...
        embedded = F.upsample(
            embedded, scale_factor=self.scale_factor, mode='nearest')
...
class Generator(nn.Module):
    def __init__(
        ...
        self.embedding = nn.Conv1d(in_channels, in_channels, 3, 1, 1, bias=False)
